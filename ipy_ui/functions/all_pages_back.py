        ...
        try:
            ...
            if self.time == formatTimeServer.strftime('%d.%m.%Y'):
                ...
                if timecheck_data < formatTimeServer_ or timecheck_data > formatTimeServer_:
                    ...
                    # Библиотека для связи с PostgreSQL не работает в
                    # многопоточном режиме, поэтому создана отдельная
                        ...
                        if subs[4] is None:
                            ...
                            try:
                                ...
                                if govCheckDate == formatTimeServer_:
                                    print(f'Госпроверка {subs[4]} == {formatTimeServer.strftime("%d.%m.%Y")} Cервер')

                                    # self.checkPostgreSQL("UPDATE ipy_names SET status_crnt_date = '{0}' WHERE state_verification = '{1}' RETURNING *;".format('вышел из строя', subs[4]))
                                    ...
                                    # self.checkPostgreSQL("UPDATE ipy_names SET status_crnt_date = '{0}' WHERE state_verification = '{1}';".format('вышел из строя', subs[4]))
                    ...
                    # Окно с сообщением.
                    successWindow(
                        message='Обновление дат выполнено : {0}'.format(formatTimeServer.strftime('%d.%m.%Y')))
