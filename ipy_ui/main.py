...
class RegistrPage(QWidget):
    ...
    def sendMessageSMTP(self):
        ...
        identificator = LoginWindow.uuid_generate(username=message)
        time = datetime.now()
...
class LoginWindow(QWidget):
    ...
    def verifyUsername(self):
        try:
            ...
            identificator = str(LoginWindow.uuid_generate(username=username_local))
            ...
            # Запись в лог
            error('[!] [EXCEPTION] Error login page, more info : {0}'.format(ex), exc_info=True)
    ...
    @staticmethod
    def uuid_generate(username) -> UUID:
        return uuid3(NAMESPACE_DNS, username)


if __name__ == '__main__':
    # Логирование всех искл в файл
